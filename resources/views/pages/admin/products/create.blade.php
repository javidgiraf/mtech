                    ...
                    <form class="row g-3" action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" id="productForm">
                        ...
                        <div class="col-12">
                            ...
                            <select name="sector_ids[]" class="form-control sector_ids @error('sector_ids') is-invalid @enderror" multiple>
                                ...
                                <option {{ in_array($sector->id, collect(old('sector_ids'))->toArray()) ? 'selected' : '' }} value="{{ $sector->id }}">{{ $sector->title }}</option>
                                @endforeach
                            </select>
                            @error('sector_ids')
                            <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="col-12">
                            <table class="table table-striped" id="applicationPhotosTable">
                                ...
                                <tbody>
                                    <tr>
                                        <th colspan="3">{{ __('Application Photos') }}</th>
                                    </tr>
                                    <tr class="keyRow" data-key="0">
                                        <td>
                                            <input type="text" name="applicationTitle[]" class="form-control @error('applicationTitle.*') is-invalid @enderror" placeholder="{{ __('Title') }}"></>
